...
/** Yaw magnitude below which the front overlay is shown uncropped. */
const EAR_CLIP_YAW_START = 8;
/** Yaw magnitude at which the clip boundary reaches its real computed position. */
const EAR_CLIP_YAW_FULL = 24;

export interface EarClip {
  side: "left" | "right";
  /** Fraction (0..1) of the overlay's own local width that stays visible on that side. */
  visibleFraction: number;
}

/**
 * Where to clip the front overlay so the temple arm on the side rotating
 * away tucks behind the ear/head instead of floating over hair/skin at a
 * flat, un-occluded depth. Returns null when the head is close enough to
 * frontal that no clipping is needed. Shared numeric core for both the live
 * CSS clip-path (`computeEarClipPath`) and the capture canvas
 * (`VirtualTryOnModal`'s `ctx.clip()`), so the saved photo matches what was
 * on screen.
 *
 * `pose.earBoundaryL/R` is a single representative point per side (see
 * faceGeometry.ts), projected onto the face's own eye-line axis — the same
 * ux/uy technique `computeOverlayPlacement` already uses for its offsetX/Y
 * — to get its position as a fraction of the overlay's own local width (the
 * frame this applies in *before* the element's own rotateZ/Y/X transform).
 */
export function computeEarClip(
  pose: FacePose,
  placement: OverlayPlacement,
  video: { videoWidth: number; videoHeight: number },
  display: { width: number; height: number }
): EarClip | null {
  const mag = Math.abs(placement.yawDeg);
  if (mag <= EAR_CLIP_YAW_START) return null;
  if (!(placement.widthPx > 0)) return null;

  const { scale } = computeCoverTransform(video, display);
  const rollRad = (pose.roll * Math.PI) / 180;
  const ux = Math.cos(rollRad);
  const uy = Math.sin(rollRad);

  const fractionAlong = (pt: { x: number; y: number }) => {
    const dx = (pt.x - pose.anchorX) * scale;
    const dy = (pt.y - pose.anchorY) * scale;
    const along = dx * ux + dy * uy;
    return 0.5 + along / placement.widthPx;
  };

  // Ease the clip boundary in over a small yaw band rather than snapping it
  // in at one angle, which would visibly pop.
  const t = clamp((mag - EAR_CLIP_YAW_START) / (EAR_CLIP_YAW_FULL - EAR_CLIP_YAW_START), 0, 1);

  if (placement.yawDeg > 0) {
    const real = fractionAlong(pose.earBoundaryR);
    return { side: "right", visibleFraction: clamp(1 + (real - 1) * t, 0, 1) };
  }
  const real = fractionAlong(pose.earBoundaryL);
  return { side: "left", visibleFraction: clamp(real * t, 0, 1) };
}

/** CSS `clip-path` value for an `EarClip`, or undefined when there's nothing to clip. */
export function earClipToCssPath(clip: EarClip | null): string | undefined {
  if (!clip) return undefined;
  return clip.side === "right"
    ? `inset(0 ${((1 - clip.visibleFraction) * 100).toFixed(2)}% 0 0)`
    : `inset(0 0 0 ${(clip.visibleFraction * 100).toFixed(2)}%)`;
}

/** CSS `clip-path` built from `computeEarClip`, or undefined when no clip is needed. */
export function computeEarClipPath(
  pose: FacePose,
  placement: OverlayPlacement,
  video: { videoWidth: number; videoHeight: number },
  display: { width: number; height: number }
): string | undefined {
  return earClipToCssPath(computeEarClip(pose, placement, video, display));
}
